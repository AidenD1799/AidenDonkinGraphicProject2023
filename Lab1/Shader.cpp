...
void Shader::initialiseGeoShader(const std::string& vertFile, const std::string& fragFile, const std::string& geoFile)
{
	shaderID = glCreateProgram();
	shaders1[0] = CreateShader(LoadShader(vertFile), GL_VERTEX_SHADER);
	shaders1[1] = CreateShader(LoadShader(geoFile), GL_GEOMETRY_SHADER);
	...
	glBindAttribLocation(shaderID, 1, "TextCoord");
	glBindAttribLocation(shaderID, 2, "VertexNormal");

	glLinkProgram(shaderID);
	CheckShaderError(shaderID, GL_LINK_STATUS, true, "Error: Shader Linking Program Failed");
